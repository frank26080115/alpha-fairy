...
TFT_eSprite* lepton_imgBuff;
bool smallBuffer_Lock = false;
bool img_buffer_Lock = false;
lepton_encoder_t lepton_encoder = {0};
const float kEncoderTempStep = 0.05f;

uint16_t lepton_histoBuff[64] = {0};
int32_t lepton_measMode = MES_AUTO_MAX;
int32_t lepton_dispMode = DISP_MODE_CAM;

/** @brief  Draws temperature focus cursor
  * @param  coordinates
  */
void IRAM_ATTR lepton_dispCursor(uint16_t x, uint16_t y)
{
...
/** @brief  Update encoder data
  ...
  */
void IRAM_ATTR lepton_updateEncoder(lepton_encoder_t* encdata)
{
    int16_t d;
    bool b = fencoder.getButtonStatus();

    if (fencoder.avail())
    {
        fencoder.task();
        d = fencoder.read(true);
        encdata->sw = fencoder.getButtonStatus();
    }
    else
    {
    ...
    if (encdata->sw) // button press resets the data
    {
        encdata->data = 0;
    }
    else
    {
        ...
        if (encdata->increment != 0)
        {
            encdata->data += ((encdata->increment) * kEncoderTempStep);
        }
    }
}
...
/** @brief  Update frame
  */
void IRAM_ATTR lepton_updateFlir()
{
    lepton_updateEncoder(&lepton_encoder);
    ...
    bool dir_flag = lepton_encoder.data >= 0;
    if (dir_flag)
    {
        if (lepton_encoder.data > 0.95)
        {
            lepton_encoder.data = 0.95;
        }

        cursor_temp = min_temp + (max_temp - min_temp) * lepton_encoder.data;
    ...
    else
    {
        ...
        cursor_temp = max_temp - ((max_temp - min_temp) * (-lepton_encoder.data));
        raw_cursor = (cursor_temp + 177.77 - fpa_temp_f) / 0.0217f;
    ...
    switch (lepton_measMode)
    {
        case MES_AUTO_MAX:
            ...
            lepton_imgBuff->printf("%.2f", max_temp);
            break;
        ...
        case MES_AUTO_MIN:
            ...
            if (min_y > FLIR_WINDOW_Y2 - 15)
                y = min_y - 15;
            lepton_imgBuff->setCursor(x, y);
            lepton_imgBuff->printf("%.2f", min_temp);
            break;
        ...
        case MES_CENTER:
            ...
            lepton_imgBuff->printf("%.2f", center_temp);
            break;
    }
...
{
    public:
        PageLeptonMesMode() : FairyCfgItem("Meas. Mode", (int32_t*)(lepton_measMode), 0, 2, 1, TXTFMT_NONE)
        {
            _autosave = false;
        };

        void on_readjust(void)
        {
    ...
    protected:
        void draw_info(void)
        {
            ...
            {
                case MES_AUTO_MAX:
                    M5Lcd.print("Auto Max Temp");
                    break;
                case MES_AUTO_MIN:
                    M5Lcd.print("Auto Min Temp");
                    break;
                case MES_CENTER:
                    M5Lcd.print("Center");
                    break;
...
{
    public:
        PageLeptonDispMode() : FairyCfgItem("Disp. Mode", (int32_t*)(lepton_dispMode), 0, 4, 1, TXTFMT_NONE)
        {
            _autosave = false;
        };
...
{
    public:
        AppLepton() : FairyCfgApp("/lepton.png", "/flir_icon.png", 0)
        {
